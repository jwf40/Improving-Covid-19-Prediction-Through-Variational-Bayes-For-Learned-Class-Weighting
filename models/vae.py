...
class vae(nn.Module):
    def __init__(self, input_channels, fc_size, encode_size, num_classes, class_latent_size, lambda_val):
        ...
        # # Concatenate into Fully Connected layer
        input_channels = [256, 512, 1024, 2048]
        self.preprocess_batch_norms = nn.ModuleList([nn.BatchNorm1d(input_channel) for input_channel in input_channels])
        self.preprocess_fcs = nn.ModuleList([nn.Linear(input_channel, encode_size) for input_channel in input_channels])
        # Encoder Architecture
        # # First Layer Input = Encode size * number of preprocess fully connected sections
        self.encoder_fc11 = FCLayer(encode_size*len(self.preprocess_fcs), fc_size)
        ...
        self.decoder_fc2 = FCLayer(fc_size, encode_size*len(self.preprocess_fcs), activation='sigmoid')
    ...
    def loss_fnc (self, recon_x, z, x, mu, logvar, targets):
        ...
        L_class = self.criterion(-z_class, targets)
    ...
    def estimate_likelihood(self, features):
        ...
        for i in range(self.num_classes):
            ...
            KLD_masked = torch.sum(KLD_element_masked, dim=1).mul_(-0.5)
